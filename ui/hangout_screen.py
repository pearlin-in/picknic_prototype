...
class HangoutScreen:
    def __init__(self, state):
        ...
        self.questions = [
            ("What's your mood today?", [
             "😌 Chill", "🎉 Fun", "🧠 Intellectual", "🏄 Adventure"]),
            ("Who's coming with you?", [
             "Just me/+1", "Small group", "Big group"]),
    ...
    def show_question(self):
        ...
        tk.Label(self.root, text=q, font=("Courier", 16, "bold"),
                 bg=bg, fg=text).pack(pady=30)
        frame = tk.Frame(self.root, bg=bg)
        frame.pack()

        for opt in options:
            tk.Button(frame, text=opt, font=("Courier", 12), bg=accent, fg=text, width=25,
                      pady=10, command=lambda o=opt: self.record_answer(o)).pack(pady=5)
    ...
    def show_results(self):
        ...
        tk.Label(self.root, text="Your Matches", font=(
            "Courier", 20, "bold"), bg=bg, fg=text).pack(pady=20)
        tk.Label(self.root, text="Top hangout spots for you:",
        ...
        for stamp in matches:
            ...
            tk.Button(frame, text=s, font=("Courier", 10),
                      bg=self.state.colors["secondary"], fg="white", command=lambda s=stamp: self.select_stamp(s)).pack(pady=5)
