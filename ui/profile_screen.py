...
class ProfileScreen:
    ...
    def show(self):
        ...
        tk.Label(self.root, text=f"👤 {self.state.current_user}'s Profile", font=(
        ...
        tk.Button(self.root, text="Upload Profile Picture", font=("Courier", 10),
                  bg=self.state.colors["accent"], command=self.upload_pic).pack(pady=10)

        tk.Label(self.root, text=f"Memories Collected: {len(self.state.memories)}", font=(
            "Courier", 12), bg=bg, fg=text).pack(pady=5)
        tk.Label(self.root, text=f"Stamps Visited: {len(set(m['stamp_id'] for m in self.state.memories))}", font=(
            "Courier", 12), bg=bg, fg=text).pack(pady=5)
