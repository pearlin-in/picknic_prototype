...
class PicknicApp:
    ...
    def show_explore(self):
        ...
        self.main_frame.explore_container = tk.Frame(
            self.main_frame, bg=self.colors["background"])
        self.main_frame.explore_container.pack(
        ...
        canvas = tk.Canvas(
            container, bg=self.colors["background"], highlightthickness=0)
        ...
        # Get all user files
        user_files = [f for f in os.listdir() if f.startswith(
            "user_") and f.endswith(".json")]
        ...
        for user_file in user_files:
            try:
                ...
                user_card.grid(row=row, column=col, padx=10,
                               pady=10, sticky="nsew")
                ...
                if profile_pic_path and os.path.exists(profile_pic_path):
                    try:
                        ...
                        pic_label = tk.Label(
                            user_card, image=photo, bg="white")
                        pic_label.image = photo
                        pic_label.pack()
                    except:
                        self.create_default_profile_display(
                            user_card, user_data["username"])
                else:
                    self.create_default_profile_display(
                        user_card, user_data["username"])
                ...
                self.update_friend_request_button(
                    request_btn, user_data["username"])
    ...
    def send_friend_request(self, target_username):
        ...
        try:
            ...
            if self.current_user not in target_data.get("friend_requests", []):
                target_data.setdefault(
                    "friend_requests", []).append(self.current_user)
                ...
                messagebox.showinfo(
                    "Success", f"Friend request sent to {target_username}!")
    ...
    def start_joint_quiz(self):
        ...
        if not self.current_user or not self.user_data.get("friends"):
            messagebox.showinfo(
                "Info", "You need friends to plan a joint hangout!")
            return
    ...
    def show_joint_question(self):
        ...
        user = "Your" if self.joint_plan[
            "current_user"] == "you" else f"{self.joint_plan['friend']}'s"
        tk.Label(
            self.main_frame,
    ...
    def show_joint_results(self):
        ...
        combined_answers = {
            "vibe": [
                self.joint_plan["your_answers"].get(
                    "Preferred vibe for the hangout?"),
                self.joint_plan["friend_answers"].get(
                    "Preferred vibe for the hangout?")
            ],
            "time": max(
                self.joint_plan["your_answers"].get(
        ...
        if not recommendations:
            ...
            recommendations = random.sample(
                self.stamps, min(3, len(self.stamps)))

        rec_frame = tk.Frame(self.main_frame, bg=self.colors["background"])
        rec_frame.pack(pady=20, fill=tk.BOTH, expand=True)

        canvas = tk.Canvas(
            rec_frame, bg=self.colors["background"], highlightthickness=0)
        scrollbar = ttk.Scrollbar(
            rec_frame, orient="vertical", command=canvas.yview)
        scrollable_frame = tk.Frame(canvas, bg=self.colors["background"])
    ...
    def save_joint_plan(self, stamp):
        ...
        try:
            ...
            messagebox.showerror(
                "Error", f"Could not save friend's plan: {str(e)}")
